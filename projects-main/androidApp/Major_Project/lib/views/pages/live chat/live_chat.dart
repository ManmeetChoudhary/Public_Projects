...
class _LiveChatState extends State<LiveChat> {
  ...
  void initState() {
    ...
    liveChatStream = _db.streamLiveChatMessages(radius: 1, currentLocation: context.read<LocationData>());
    //TODO: you'll have to make a new stream on location change
...
class _ChatFieldState extends State<ChatField> {
  ...
  void initState() {
    super.initState();
    //textBubbles.add(new TextBubble("big test text\n1\n", Position(latitude: widget.location.latitude + 1, longitude: widget.location.longitude), DateTime.now()));
    ...
    //textBubbles.add(new TextBubble("east", Position(latitude: widget.location.latitude, longitude: widget.location.longitude + 1), DateTime.now()));
    //textBubbles.add(new TextBubble("south", Position(latitude: widget.location.latitude - 1, longitude: widget.location.longitude), DateTime.now()));
    //textBubbles.add(new TextBubble("west", Position(latitude: widget.location.latitude, longitude: widget.location.longitude - 1), DateTime.now()));

    //textBubbles.add(new TextBubble("test", Position(latitude: widget.location.latitude, longitude: widget.location.longitude), DateTime.now()));
    //textBubbles.add(new TextBubble("test 2", Position(latitude: widget.location.latitude, longitude: widget.location.longitude), DateTime.now()));
    //textBubbles.add(new TextBubble("test                 3", Position(latitude: widget.location.latitude, longitude: widget.location.longitude), DateTime.now()));
    //textBubbles.add(new TextBubble("test\n4\ntall\nmessage", Position(latitude: widget.location.latitude, longitude: widget.location.longitude), DateTime.now()));
    ...
    for (int i = 0; i < textBubbles.length; i++) {
      if (hasChild(i)) {
        if (textBubbles[i].direction != -1) { //calculate positions for textBubbles in outer-ring
          ...
          if ((overlapping.length == 0) && (textBubbles[i].bufferPosition.dLocation != textBubbles[i].displayPosition)) {
...
class TextBubble extends spaceBlock {
  ...
  TextBubble(this.text, this.location, this.postTime) : super(true) {
    ...
    Timer(collapseAt, () { this.displayable = true; });
  }
}
